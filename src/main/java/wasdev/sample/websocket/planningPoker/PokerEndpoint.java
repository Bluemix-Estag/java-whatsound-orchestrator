...
@ServerEndpoint(value = "/TesteEndpoint")
public class PokerEndpoint {
	...
	public void onOpen(Session session, EndpointConfig ec) {
		...
		service.setUsernameAndPassword("d2c8d8f6-5a19-44ea-be21-c34177a50652", "rmj1NwXbmkCR");
		MessageRequest newMessage = new MessageRequest.Builder().inputText("Ola").build();
		//"9e2ff96c-4a5e-4520-8590-dc0d0cc1e9cf"
		MessageResponse response = service.message("660dfa56-12ec-409a-b6a8-dbad53eb9026", newMessage).execute();
	...
	public void receiveMessage(String message) {
		if(message.equals("")){
			...
			try {
				...
				if(currentDialog.getCurrentResponse().getContext().get("action").toString().equals("google/spotify")){
					...
					if(query.getType() == "spelling"){
						...
						currentDialog.getCurrentResponse().getContext().put("typeGoogle", newquery.getType());
					}
						...
						currentDialog.getCurrentResponse().getContext().put("typeGoogle", query.getType());
					}
					
					if(completeResponse[0] != ""){
						System.out.println(currentDialog.getSearchType());
						System.out.println(query.getType());
						if (query.getType().equals(currentDialog.getCurrentResponse().getContext().get("typeUser").toString())){
							System.out.println(spotify.getTrackResult());
					...
					getNewResponse(currentMessage);
					String response = currentDialog.getText().replace("artistname", spotify.getArtistResult()).replace("trackname", spotify.getTrackResult());
					currentSession.getBasicRemote().sendText(buildMessage(response,"Watson"));
